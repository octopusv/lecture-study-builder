...
- **コード**（`scripts/`、`.claude/skills/`、`portal.css`、テンプレートHTML、各種スクリプト等）は **[PolyForm Noncommercial License 1.0.0](./LICENSE)** で提供します。個人・教育・研究などの**非営利目的の利用は自由**です。
- これは**ソースアベイラブル**ライセンスです。全ソースを公開していますが、商用利用を制限するため**OSI承認のオープンソースではありません**。
- **第三者の商用利用には、著作権者からの個別ライセンスが必要**です。商用利用を希望する場合は作者（octopusv）へ連絡してください。
- 著作権者（octopusv）は著作権者本人として、商用利用を含む全ての権利を留保します。
- **公開する生成物**（`deploy/public` の学習サイト等、別途公開する成果物）は **CC BY-NC 4.0** を想定します。
- 本ライセンスが及ぶのは**このリポジトリのコード・ドキュメントのみ**です。授業動画や、それを基に生成した学習資料は元の権利者に権利が残り、リポジトリにも含まれません（Git管理外）。
- 商用権を明確に保つため、本リポジトリは原則として外部からのコード貢献を受け付けません（取り込む場合はCLAで権利関係を明確化します）。
